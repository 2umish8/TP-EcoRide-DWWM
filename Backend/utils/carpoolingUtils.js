 ...
 */
const cancelCarpoolingById = async (carpoolingId, options = { isAutoCancelled: false }) => {
    try {
        const result = await prisma.$transaction(async (transactionPrisma) => {
            ...
            if (carpooling.status !== "prévu") {
                throw new Error("Seuls les covoiturages prévus peuvent être annulés.");
            }
        ...
        console.error(`❌ Erreur annulation covoiturage #${carpoolingId}:`, error.message);
        return {
            success: false,
            participantsCount: 0,
            error: error.message,
