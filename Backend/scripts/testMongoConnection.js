...
async function testMongoConnection() {
    ...
    try {
        ...
        console.log(
            `✅ Modèle Review accessible - ${reviewCount} reviews trouvées`
        );
        ...
        console.log(
            `✅ ${reviews.length} reviews récupérées pour l'utilisateur 109`
        );
        ...
        if (reviews.length > 0) {
            ...
            console.log(
                `   - Commentaire: ${reviews[0].comment?.substring(0, 50)}...`
            );
        }
        ...
        console.log(
            `✅ ${pendingReviews.length} reviews en attente de validation`
        );
