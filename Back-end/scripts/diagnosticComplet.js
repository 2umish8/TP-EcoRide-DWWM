...
const API_BASE_URL = process.env.API_URL || 'https://tp-ecoride-dwwm-production.up.railway.app/api';
...
async function diagnosticComplet() {
    ...
    try {
        ...
        if (response.data.stats) {
            ...
            console.log(`   - Note moyenne: ${response.data.stats.averageRating}`);
            console.log(`   - Total reviews: ${response.data.stats.totalReviews}`);
        } else {
            console.log('❌ Statistiques manquantes dans la réponse');
        }
        
        if (response.data.reviews) {
            console.log(`✅ Reviews présentes: ${response.data.reviews.length} reviews`);
